...
function Switch({
	...
	return (
		<SwitchPrimitive.Root
			...
			className={cn(
				FOCUS_RING,
				"data-checked:bg-primary data-unchecked:bg-input dark:data-unchecked:bg-input/80 hover:ring-2 hover:ring-ring/30 shrink-0 rounded-[calc(var(--radius)+2px)] p-0.5 data-[size=default]:h-4 data-[size=default]:w-7 data-[size=sm]:h-3.5 data-[size=sm]:w-6 peer group/switch relative inline-flex items-center transition-[opacity,box-shadow] outline-none after:absolute after:-inset-x-3 after:-inset-y-2 data-disabled:cursor-not-allowed data-disabled:opacity-disabled",
				className,
			)}
			{...props}
		>
