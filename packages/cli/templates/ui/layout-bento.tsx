...
const BENTO_CONTAINER =
	"bg-[var(--bento-border-color,var(--border))] gap-[var(--bento-gap,1px)] p-[var(--bento-gap,1px)] rounded-[var(--bento-radius,var(--radius))]"

/** Every direct cell child gets card background — it sits on top of the border bg. */
const BENTO_CELL = "bg-card rounded-[var(--bento-radius,var(--radius))]"
	...
	return (
		<div
			...
			className={cn(
				"grid grid-cols-1",
				CELL_GRID_COLS[cols],
				"gap-[var(--bento-gap,1px)]",
				subtle
					? "bg-[var(--border-secondary,var(--border))]"
					: "bg-[var(--bento-border-color,var(--border))]",
				className,
			)}
			{...props}
		/>
